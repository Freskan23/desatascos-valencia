...
// Protege /dashboard (sesión) y /api/admin + /dashboard/editor + /dashboard/settings (solo admin).
...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  const adminOnly = path.startsWith('/api/admin') || path.startsWith('/dashboard/editor') || path.startsWith('/dashboard/settings');
